...
<template>
  <BModal @hidden="$emit('close-modal')" body-class="pb-0" content-class="border-0"
    footer-class="justify-content-between" :id="modalId" no-close-on-backdrop no-close-on-esc scrollable size="lg">
    ...
    <!-- Content -->
    <template v-else>
      <BTable class="mb-0" borderless thead-class="d-none" :current-page="commentsCurrentPage" :items="comments"
        :fields="commentsFields" :per-page="commentsItemsPerPage">
        <template #cell(icon)>
          <div class="d-flex flex-column mr-4 position-absolute h-100 comment-icon">
            <div class="rounded-circle bg-light d-flex align-items-center justify-content-center py-2">
              <FrIcon name="chat_bubble_outline" icon-class="md-18" />
            </div>
        ...
        <template #cell(comment)="{ item }">
          <div class="media-body my-2 pr-5 border-bottom">
            <BMedia class="align-items-center mb-2" no-body>
              <BImg v-if="item.user" class="mr-3 rounded-circle" height="16" width="16"
              ...
              <div class="media-body">
                <div class="text-dark d-flex justify-content-start">
                  <div class="font-weight-bold" v-if="item.user">
      ...
      <FrPagination v-model="commentsCurrentPage" boundary="scrollParent" hide-border :per-page="commentsItemsPerPage"
        :total-rows="comments.length" @input="commentsPaginationChange" @on-page-size-change="commentsPageSizeChange" />
    </template>
    <template #modal-footer="{ cancel }">
      <BButton variant="outline-primary" @click="openAddCommentModal()">
